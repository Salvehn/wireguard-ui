...
if (process.getuid() !== 0 || !Number.isSafeInteger(uid) || uid < 1)
  process.exit(1);
...
const core = createCore({ base: __dirname, configDirectory, userId: uid });
...
(async () => {
  ...
  if (
    ...
    throw Error("Unsafe runtime directory");
  ...
  const server = net.createServer((socket) => {
    ...
    socket.on("data", (chunk) => {
      ...
      const job = tail.then(async () => {
        try {
          const result = await core.handle(JSON.parse(body.trim()));
  ...
  await fs.chown(socketPath, uid, 0);
