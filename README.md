...
**Smart tunneling** lets each connection use its original routes, only listed domains/IP networks, or routes excluding the list. Rules narrow the original `AllowedIPs`; domains resolve when connecting. List modes use system DNS. Per-app routing is not supported.
