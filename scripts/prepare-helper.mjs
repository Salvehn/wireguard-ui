...
import {prepareBash} from './prepare-bash.mjs';
...
const [singBoxRoot,bash]=await Promise.all([prepareSingBox(),prepareBash()]);
const sources={node,bash:bash.binary,wg:prefix+'/bin/wg','wg-quick':prefix+'/bin/wg-quick','wireguard-go':prefix+'/bin/wireguard-go','sing-box':path.join(singBoxRoot,'sing-box')};
...
const licenses={Node:path.resolve(path.dirname(node),'../LICENSE'),Bash:bash.license,WireGuardTools:prefix+'/opt/wireguard-tools/COPYING',WireGuardGo:prefix+'/opt/wireguard-go/LICENSE'};
for(const [name,file] of Object.entries(licenses))await fs.copyFile(file,path.join(base,'licenses',name+'.txt'));
const bashSource=path.join(base,'licenses','Bash-source');await fs.rm(bashSource,{recursive:true,force:true});await fs.mkdir(bashSource,{recursive:true});
for(const file of bash.sourceFiles)await fs.copyFile(file,path.join(bashSource,path.basename(file)));
await fs.writeFile(path.join(bashSource,'BUILD.txt'),'Apply bash53-001 through bash53-015 to bash-5.3, then configure with --disable-nls --disable-readline --without-bash-malloc and MACOSX_DEPLOYMENT_TARGET=11.0.\n');
await fs.writeFile(path.join(base,'licenses/SOURCES.txt'),`Node.js: https://nodejs.org/\nBash ${bash.version}: https://ftp.gnu.org/gnu/bash/\nWireGuard tools: https://git.zx2c4.com/wireguard-tools/\nWireGuard Go: https://git.zx2c4.com/wireguard-go/\n`);
